...
function isIncomeTransaction(description: string): boolean {
    const incomeKeywords = [
        'transfer', 'inward', 'deposit', 'reverse charges', 
        'atm cash deposit', 'cash deposit', 'credit'
    ];
...
function parseTransaction(line: string): ParsedTransaction | undefined {
    ...
    for (let i = 0; i < transactionPatterns.length; i++) {
        ...
        if (match) {
            let dateStr: string, description: string, amountStr: string, balanceStr: string;
            
            if (i === 3) { // VAT pattern
                [, dateStr, , description, amountStr, , balanceStr] = match;
            } else if (i === 4) { // Switch/ATM pattern
                [, dateStr, , description, amountStr, balanceStr] = match;
    ...
    if (continuationMatch && pendingTransaction) {
        const [, additionalDesc, balanceStr] = continuationMatch;
        ...
        if (amountMatch && !pendingTransaction.amount) {
        ...
        if (pendingTransaction.date && pendingTransaction.description && pendingTransaction.amount !== undefined) {
    ...
    if (incompleteMatch) {
        const [, dateStr, , description] = incompleteMatch;
        const date = parseDate(dateStr);
